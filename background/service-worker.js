...
async function openChatGptTabForRequest(requestId) {
  ...
  const tabId = request?.chatTabId || (await findOrCreateChatGptTab({ sourceWindowId: request?.source?.windowId })).id;
...
async function orchestrateRequest(requestId, { attachments = [], adapterHints = [] } = {}) {
  try {
    const request = await getRequest(requestId);
    ...
    const chatTab = await findOrCreateChatGptTab({
      sourceWindowId: request.source.windowId
    });
...
async function findOrCreateChatGptTab({ sourceWindowId } = {}) {
  ...
  for (const tab of candidates) {
    const probe = await probeChatGptTab(tab.id);
...
async function queryBestSourceTab() {
  ...
  if (lastFocusedTab && !isExtensionUrl(lastFocusedTab.url || "")) {
