...
assert.equal(runBridgeSetupForOrigin("chrome-extension://test-extension"), true);
assert.equal(runBridgeSetupForOrigin("moz-extension://test-extension"), true);
assert.equal(runBridgeSetupForOrigin("https://example.com"), false);

console.log("Offscreen bridge origin tests passed.");

function runBridgeSetupForOrigin(extensionOrigin) {
  const messages = [];
  const ports = [];
  const topWindow = {};
  const location = {
  ...
  return ports.length === 1 && messages.some((message) => {
